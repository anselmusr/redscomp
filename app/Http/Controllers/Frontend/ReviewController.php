...
class ReviewController extends Controller
{
    ...
    {
        $product = Product::where('name', $product_name)->where('status', '1')->first();
    ...
    public function create(Request $request)
    {
        ...
        if ($product) {
            ...
            if ($new_review) {
                return redirect('category/' . $category_name . '/' . $prod_name)->with('status', "Thankyou for writing a review");
            }
        } else {
            return redirect()->back()->with('status', "The Link you followed was broken");
        }
    ...
    public function update(Request $request)
    {
        ...
        if ($user_review != '') {
            ...
            if ($review) {
                ...
                $review->update();
                return redirect('category/' . $review->product->category->name . '/' . $review->product->name)->with('status', "Review Updated Successfully");
            } else {
                return redirect()->back()->with('status', "The Link you followed was broken");
            }
        } else {
